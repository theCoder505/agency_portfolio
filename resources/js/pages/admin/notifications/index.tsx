...
export default function AdminNotificationsIndex({
    ...
    const handleSearchSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        handleFilterChange(currentStatus, currentType);
    };
    ...
    const handleClearAllRead = async () => {
        const confirmed = await showConfirmDialog(
            ...
            'This will permanently delete all notifications you have already read.',
            'Yes, clear them',
            'warning'
        );
        ...
        if (confirmed) {
            router.delete('/admin/notifications/clear-read', {
                onSuccess: () => showToast('Read notifications cleared', 'success'),
            });
        }
    };
    ...
    const tabs = [
        ...
        { label: 'Custom Orders', key: 'order', type: 'order', count: typeCounts?.order || 0 },
        { label: 'SaaS Subscriptions', key: 'subscription', type: 'subscription', count: typeCounts?.subscription || 0 },
        { label: 'Payments', key: 'payment', type: 'payment', count: typeCounts?.payment || 0 },
        { label: 'Contact Inquiries', key: 'contact', type: 'contact', count: typeCounts?.contact || 0 },
        { label: 'Client Reviews', key: 'review', type: 'review', count: typeCounts?.review || 0 },
    ];
    ...
    return (
        ...
        >
            <Head title="Notification Center - Admin Panel" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                            Real-time notices and audit log for orders, subscriptions, payment proofs, contacts, and client reviews.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center space-x-2.5 flex-wrap">
                        {unreadCount > 0 && (
                            <button
                                ...
                                className="inline-flex items-center space-x-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-cyan-400 border border-indigo-200/60 dark:border-indigo-800/60 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition-all"
                            >
                        ...
                        <button
                            ...
                            className="inline-flex items-center space-x-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-950/40 dark:hover:text-rose-400 border border-slate-200/80 dark:border-slate-700/80 transition-all"
                        >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <form onSubmit={handleSearchSubmit} className="relative w-full md:w-72">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2 text-xs rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                            />
                        </form>
                    </div>
                    ...
                    {notifications.data.length === 0 ? (
                        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-12 text-center shadow-xs">
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto mb-4">
                                There are no notifications matching your active filters. When customers place orders, make payments, submit inquiries, or post reviews, they will appear here.
                            </p>
                            {(currentStatus !== 'all' || currentType !== 'all' || search) && (
                                <button
                                    type="button"
                                    onClick={() => {
                                        setSearch('');
